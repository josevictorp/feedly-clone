...
**No meio:**
- Brainstorm classificado como arquitetural. Primeira pergunta feita ao usuário (forma do produto: app desktop, web em localhost com servidor local, ou só navegador). Usuário respondeu B: web app em localhost com servidor local. Q2 (ordem dos sub-projetos) aprovada: leitor base → organização premium → Leo → descoberta. Q3: capturas concluídas (docs/research/README.md). Q4: stack sem restrições. Q5: UI em português. Q6 aceite aprovado. Abordagem A escolhida. Apresentando design em seções (1 arquitetura/pastas, 2 modelo de dados, 3 motor de feeds, 4 API, 5 front/design system, 6 i18n e preferências, 7 erros, 8 testes). Seção 1 enviada; aguardando aprovação.
...
3. Escrever spec em `docs/superpowers/specs/2026-09-16-feedly-clone-design.md` (repositório git já existe).
4. Rodar grill-me sobre a spec.
5. Escrever PRD em `docs/prd/`.
6. Esperar aprovação do usuário. Só então executar.
