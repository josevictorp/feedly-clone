...
- pnpm workspace, `.nvmrc` (26), TypeScript estrito, ESLint, Prettier.
- `apps/server` com Hono servindo `/api/health` e estáticos; `apps/web` com Vite e uma página em branco com o título "Feedly"; `packages/shared` vazio com build.
- Scripts: `pnpm dev`, `pnpm build`, `pnpm start` (abre o navegador), `pnpm test`, `pnpm lint`, `pnpm typecheck`.
- Vitest e Playwright configurados; um teste de cada rodando.
- `data/` com `FEEDLY_DATA_DIR` e `PORT`.
- README com "como rodar"; AGENTS.md com a stack.
...
- Schema Drizzle das tabelas da spec (feeds, categories, feed_categories, entries, stream_settings, preferences) com índices.
- Migrações no boot com backup do arquivo antes de migrar.
- Preferências com valores padrão semeados na primeira execução.
- Repositórios: feeds, categories, entries, streamSettings, preferences, com as consultas agregadas de sidebar e streams (cursor).
- Job de retenção (função pura + agendamento diário).
